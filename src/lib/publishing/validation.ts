...
export async function validatePackagePublishReadiness(drillPackage: DrillPackage): Promise<PublishReadinessResult> {
  ...
  if (versioning?.derivedFrom?.relation === "new-version" && versioning.lineageId !== drillPackage.manifest.packageId) {
    issues.push({
      severity: "warning",
      code: "lineage-conflict",
      path: "manifest.versioning.lineageId",
      message: "New version packages should preserve lineage with the base package identity."
    });
  }
