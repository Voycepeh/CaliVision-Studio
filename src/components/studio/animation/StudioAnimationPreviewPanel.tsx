...
export function StudioAnimationPreviewPanel() {
  ...
  return (
    <section className="card">
      <h3 style={{ marginTop: 0, marginBottom: "0.4rem", fontSize: "0.95rem" }}>Animation preview</h3>
      <p className="muted" style={{ marginTop: 0 }}>
        Preview uses the same canonical phase poses and duration fields that export in package JSON.
      </p>
        ...
        <div style={{ marginTop: "0.6rem", display: "grid", gap: "0.35rem" }}>
          {timeline.segments.map((segment, index) => (
            <div key={segment.phaseId} className="studio-animation-segment" data-active={index === sampledFrame.phaseIndex}>
              ...
              <span className="muted">{segment.durationMs}ms</span>
            </div>
          ))}
        </div>
      ) : null}
