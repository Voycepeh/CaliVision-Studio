...
export function AdminPanel({ canManageRoles }: AdminPanelProps) {
  ...
  async function onModeratePublication(userId: string, drillId: string, publicationId: string, action: ExchangeModerationAction): Promise<void> {
    ...
    setDrillsByUser((current) => ({
      ...current,
      [userId]: (current[userId] ?? []).map((drill) => (drill.id === drillId ? { ...drill, exchangeStatus: result.value } : drill))
    }));
  }
          ...
          return (
            <article key={user.userId} className="card" style={{ display: "grid", gap: "0.45rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.45rem", flexWrap: "wrap" }}>
                ...
                <button type="button" className="pill" disabled={!canManageRoles || loadingRole || user.role === "moderator"} onClick={() => void onSetRole(user, "moderator")}>
                  Set as moderator
                </button>
              ...
              {expandedUserId === user.userId ? (
                <section style={{ display: "grid", gap: "0.35rem" }}>
                  {drills.length === 0 ? <p className="muted" style={{ margin: 0 }}>No authored drills found.</p> : null}
                  {drills.map((drill) => (
                    <div key={`${user.userId}:${drill.id}:${drill.updatedAtIso}`} style={{ border: "1px solid var(--border)", borderRadius: "0.5rem", padding: "0.45rem" }}>
                      <strong>{drill.title}</strong>
                      <p className="muted" style={{ margin: "0.2rem 0 0" }}>
                        Status: {drill.status} • Exchange: {drill.exchangeStatus ?? "not published"} • Updated {formatDate(drill.updatedAtIso)}
                      </p>
                      {drill.publicationId ? (
                        <div style={{ display: "flex", gap: "0.4rem", marginTop: "0.4rem", flexWrap: "wrap" }}>
                          {(() => {
                            const publicationId = drill.publicationId;
                            return (
                              <>
                          <button
                            type="button"
                            className="pill"
                            disabled={pendingModerationPublicationId === publicationId}
                            onClick={() => void onModeratePublication(user.userId, drill.id, publicationId, "hide")}
                          >
                            Hide publication
                          </button>
                          <button
                            ...
                            className="pill"
                            disabled={pendingModerationPublicationId === publicationId}
                            onClick={() => void onModeratePublication(user.userId, drill.id, publicationId, "archive")}
                          >
                            Archive publication
                          </button>
                          <button
                            ...
                            className="pill"
                            disabled={pendingModerationPublicationId === publicationId}
                            onClick={() => void onModeratePublication(user.userId, drill.id, publicationId, "delete")}
                          >
                            Delete publication
                          </button>
                              </>
